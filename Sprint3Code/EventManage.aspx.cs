    ...
    public partial class EventManage : Page
    {
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            ...
            if (!IsPostBack)
            {
                ...
                string microcourse = Request.QueryString["microcourse"];
                string timeWindow = Request.QueryString["timeWindow"];

                TxtSearch.Text = search;
                DdlMicrocourse.SelectedValue = microcourse;
                DdlTimeWindow.SelectedValue = timeWindow;

                // Populate dropdown from microcourses
                BindCourseSelect();
                DdlMicrocourse.DataSource = CourseSelect.Items;
                DdlMicrocourse.DataTextField = "Text";
                DdlMicrocourse.DataValueField = "Value";
                DdlMicrocourse.DataBind();

                BindSessionsFiltered(search, microcourse, timeWindow);
        ...
        private void BindSessionsFiltered(string search, string microcourse, string timeWindow)
        {
            var rows = new List<object>();
            if (!File.Exists(EventSessionsXmlPath))
            {
            ...
            foreach (XmlElement s in doc.SelectNodes($"/eventSessions/session[@eventId='{_eventId}']"))
            {
                ...
                if (!TryParseIsoUtc(startIso, out var startUtc) || !TryParseIsoUtc(endIso, out var endUtc))
                    continue;
                ...
                if (!string.IsNullOrEmpty(search))
                    match &= (title?.IndexOf(search, StringComparison.OrdinalIgnoreCase) >= 0 ||
                              helper.IndexOf(search, StringComparison.OrdinalIgnoreCase) >= 0);

                if (!string.IsNullOrEmpty(microcourse))
                    match &= string.Equals(courseId, microcourse, StringComparison.OrdinalIgnoreCase);

                if (!string.IsNullOrEmpty(timeWindow))
                {
                    var hour = startUtc.ToLocalTime().Hour;
                    if (timeWindow == "morning") match &= hour < 12;
                    else if (timeWindow == "afternoon") match &= hour >= 12 && hour < 17;
                    else if (timeWindow == "evening") match &= hour >= 17;
                }
                ...
                {
                    courseTitle = title ?? "(untitled)",
                    startLocal = ToLocalHuman(startIso),
                    endLocal = ToLocalHuman(endIso),
                    room,
                    helper,
                    capacity = string.IsNullOrEmpty(cap) ? "—" : cap
                });
        ...
        protected void BtnFilterSessions_Click(object sender, EventArgs e)
        {
            string search = TxtSearch.Text.Trim();
            string microcourse = DdlMicrocourse.SelectedValue;
            string timeWindow = DdlTimeWindow.SelectedValue;

            // Sticky filters → persist in query string
            string qs = $"?id={_eventId}&search={Server.UrlEncode(search)}&microcourse={Server.UrlEncode(microcourse)}&timeWindow={Server.UrlEncode(timeWindow)}";
            Response.Redirect("EventManage.aspx" + qs);
        }

        protected void BtnClearFilters_Click(object sender, EventArgs e)
        {
            Response.Redirect($"EventManage.aspx?id={_eventId}");
        }
